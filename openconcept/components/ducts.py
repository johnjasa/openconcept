...
class DuctWithHxOld(Group):
    ...
    def setup(self):
        ...
        iv.add_output('Utrue', val=300.9*np.linspace(0.03,1,nn), units='kn')
        ...
        iv.add_output('area_3', val=286.918, units='inch**2')
        #iv.add_output('delta_p_3', val=np.ones((nn,))*0, units='Pa')
        iv.add_output('heat_in_3', val=np.zeros((nn,)), units='W')
...
class DuctWithHx(Group):
    ...
    def setup(self):
        ...
        iv = self.add_subsystem('dv', IndepVarComp(), promotes_outputs=['cp','*_1','*_2','*_3','*_nozzle','convergence_hack', 'T_inf', 'p_inf', 'Utrue'])
        iv.add_output('cp', val=1002.93, units='J/kg/K')
