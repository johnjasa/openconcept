...
class LinearInterpolator(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials('vec', 'start_val',
                              rows=arange, cols=np.zeros(nn), val=np.linspace(1, 0, nn))
        self.declare_partials('vec', 'end_val',
                              rows=arange, cols=np.zeros(nn), val=np.linspace(0, 1, nn))
