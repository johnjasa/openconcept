...
class OperatingCost(ExplicitComponent):
    def initialize(self):
        ...
        self.options.declare('airframe_cost_per_kg',default=266.0,desc='Cost of each kg of airplane empty weight MINUS ENGINES/MOTORS/GEN/BATTERIES')
        self.options.declare('fuel_cost_per_kg',default=2.5/3.08,desc='Cost of each KG of fuel; divide USD/gal by 3.08kg/gal')
        self.options.declare('electricity_cost_per_MJ',default=0.01,desc='Electricity cost per MJ. Divide MWh cost by 3600 MJ/MWh')
        ...
        self.options.declare('battery_replacement_cycles',default=1500,desc='Number of battery cycles until replacement')
        self.options.declare('OEM_premium',default=1.1,desc='Multiplier on the airframe cost as profit margin for the OEM. 10% margin = 1.1')
    ...
    def setup(self):
        ...
        for i in range(n_components):
            ...
            self.declare_partials(['airframe_NR_cost','total_NR_cost'], ['component_'+str(i+1)+'_weight'], val= - af_cost_kg * OEM_premium)
            self.declare_partials(['trip_direct_operating_cost'], ['component_'+str(i+1)+'_weight'], val= - af_cost_kg * OEM_premium / aircraft_daily_cycles / 365 / aircraft_life_years)
            self.declare_partials(['total_NR_cost'], ['component_'+str(i+1)+'_NR_cost'], val= OEM_premium)
            self.declare_partials(['trip_direct_operating_cost'], ['component_'+str(i+1)+'_NR_cost'], val= OEM_premium/ aircraft_daily_cycles / 365 / aircraft_life_years)
        ...
        if n_batteries is not None:
            ...
            for i in range(n_batteries):
                ...
                self.add_input('battery_'+str(i+1)+'_energy_used', units='MJ',desc='Battery energy used for mission')
                self.declare_partials(['electricity_cost','trip_energy_cost','trip_direct_operating_cost'], ['battery_'+str(i+1)+'_energy_used'], val=electricity_cost_per_MJ)
                self.declare_partials(['trip_battery_cost','trip_direct_operating_cost'], ['battery_'+str(i+1)+'_NR_cost'], val= 1 / battery_replacement_cycles )
        ...
        self.declare_partials(['trip_direct_operating_cost'], ['OEW'], val=af_cost_kg * OEM_premium / aircraft_daily_cycles / 365 / aircraft_life_years)
        self.declare_partials(['fuel_cost','trip_energy_cost','trip_direct_operating_cost'], ['fuel_burn'], val=fuel_cost_per_kg)
    ...
    def compute(self, inputs, outputs):
        ...
        if n_batteries is not None:
            ...
            outputs['trip_direct_operating_cost'] = outputs['trip_energy_cost'] + NR_contrib_trip_cost + outputs['trip_battery_cost']
        else:
