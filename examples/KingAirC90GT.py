...
class KingAirC90GTModel(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('propmodel',TwinTurbopropPropulsionSystem(num_nodes=nn),
...
if __name__ == "__main__":
    ...
    prob['rotate.CL_rotate_mult'] = np.linspace(0.83,0.79,num_nodes)
    prob.run_model()
    prob.model.analysis.descent.list_inputs(print_arrays=True,units=True)
    # list some outputs
    units=['lb','lb','lb','ft','ft']
    for i, thing in enumerate(['ac|weights|MTOW','climb.OEW','descent.fuel_used_final','rotate.range_final','v0v1.range_final']):
